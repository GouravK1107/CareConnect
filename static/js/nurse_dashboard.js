...
function processExtractedText(text) {
  ...
  if (filledFields > 5) {
    showNotification(
      `Successfully extracted data from ${filledFields} fields!`,
      "success"
    );
  } else {
    showNotification(
      "Limited data found in document. Please review and complete manually.",
      "warning"
    );
  }
}
...
function extractPrimaryPhysician(text) {
  const physicianMatch = text.match(
    /Primary Physician:\s*(Dr\.? [A-Za-z ]+?)(?=\s*(?:Date of Admission|Patient ID|$))/
  );
...
function extractAdmissionDate(text) {
  const admissionMatch = text.match(
    /Date of Admission:\s*([0-9]{1,2} [A-Za-z]{3} [0-9]{4})/
  );
  return admissionMatch ? admissionMatch[1].trim() : "";
}

function extractDischargeDate(text) {
  const dischargeMatch = text.match(
    /Date of Discharge:\s*([0-9]{1,2} [A-Za-z]{3} [0-9]{4})/
  );
...
function extractSurgeryDate(text) {
  const surgeryMatch = text.match(
    /Surgery Date:\s*([0-9]{1,2} [A-Za-z]{3} [0-9]{4})/
  );
...
function extractTreatmentSummary(text) {
  ...
  return text
    .substring(treatmentStart + "Treatment Summary".length, nextSection)
    .replace(/\n/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function extractDischargeMeds(text) {
  ...
  const medsText = text
    .substring(startIndex + "5. Medications on Discharge".length, endIndex)
    .trim()
    .split("\n")
...
function extractFollowUpPlan(text) {
  ...
  const followUpText = text
    .substring(startIndex + "6. Follow-Up Plan".length, endIndex)
    .trim()
    .split("\n")
    .filter((line) => line.trim())
    .join("\n");
...
function extractHomeCare(text) {
  ...
  const homeCareText = text
    .substring(startIndex + "7. Restrictions & Home Care".length, endIndex)
    .trim()
    .split("\n")
    .filter((line) => line.trim())
    .join("\n");
...
function extractEmergencyContacts(text) {
  ...
  const contactsText = text
    .substring(startIndex + "8. Emergency Contacts".length, endIndex)
    .trim()
    .split("\n")
    .filter((line) => line.trim())
    .join("\n");
...
function handleSubmit(event) {
  ...
  if (notificationManager) {
    notificationManager.addNotification(
      'success',
      'Patient record created successfully and added to follow-up system.',
...
class DarkModeManager {
    ...
    toggle() {
        ...
        if (window.notificationManager) {
            window.notificationManager.addNotification('info', 
